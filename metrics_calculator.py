    ...
    for run_ in range(args.runs):
        ...
        for class_index in range(args.class_index):
                ...
                if args.regression_models == 'STGCN':
                        ...
                        csv_path = stgcn_run_dir_class + f'/train_vs_noisy_vs_gen_{class_index}.csv'
                        ...
                        print('path-------------------------------------------',output_directory_skeletons_class)
                        generated_samples,gen_scores = generator.generate_samples_from_prior(device = args.device,gif_directory=stgcn_run_dir_class,dataloader=train_loader)
                ...
                #########################################################################################################################################################################""
                        ...
                        if os.path.exists(stgcn_run_dir_class + f'/train_vs_test{class_index}.csv'):
                                df = pd.read_csv(stgcn_run_dir_class + f'/train_vs_test{class_index}.csv')
                        else:
                                df = pd.DataFrame(columns=['ON','FID','COV','MMS','Density','APD'])
                        ...
                        generator = SVAE(output_directory=output_directory_skeletons_class,
                                ...
                                w_rec=weights_loss['wrec'],
                                w_kl=weights_loss['wkl'])
                        generated_samples,gen_scores = generator.generate_samples_from_prior(device = args.device,gif_directory=stgcn_run_dir_class,dataloader=test_loader)
                        gen_set = Kimore(generated_samples,labels,gen_scores)
                        gen_loader = DataLoader(gen_set,batch_size=16,shuffle =True)
                        ...
                        df.to_csv(stgcn_run_dir_class + f'/train_vs_test{class_index}.csv', index=False)
                        ...
                        if os.path.exists(reg_run_dir_class + f'/train_vs_noisy_vs_gen_{class_index}.csv'):
                                df = pd.read_csv(reg_run_dir_class + f'/train_vs_noisy_vs_gen_{class_index}.csv')
                        else:
                                df = pd.DataFrame(columns=['ON','FID','COV','MMS','Density','APD'])
                        ...
                        generator = SVAE(output_directory=output_directory_skeletons_class,
                                ...
                                w_rec=weights_loss['wrec'],
                                w_kl=weights_loss['wkl'])
                        generated_samples,gen_scores = generator.generate_samples_from_prior(device = args.device,gif_directory=reg_run_dir_class,dataloader=train_loader)
                        gen_set = Kimore(generated_samples,labels,gen_scores)
                        gen_loader = DataLoader(gen_set,batch_size=16,shuffle =True)
                        ...
                        df.to_csv(reg_run_dir_class + f'/train_vs_noisy_vs_gen_{class_index}.csv', index=False)
                #########################################################################################################################################################################""

                        if os.path.exists(reg_run_dir_class + f'/train_vs_test{class_index}.csv'):
                                df = pd.read_csv(reg_run_dir_class + f'/train_vs_test{class_index}.csv')
                        else:
                                df = pd.DataFrame(columns=['ON','FID','COV','MMS','Density','APD'])
                        ...
                        #Density metric
                        density_score = density_calculator_reg.calculate_density(gen_loader, train_loader)
                        ...
                        df.to_csv(reg_run_dir_class + f'/train_vs_test{class_index}.csv', index=False)
