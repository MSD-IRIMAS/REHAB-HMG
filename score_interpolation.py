...
if __name__ == "__main__":
    ...
    def generate_missing_floats(arr, lower_bound=0.0, upper_bound=100.0, num_to_generate=100):
        arr_set = set(arr.flatten())
        generated_count = 0
        while generated_count < num_to_generate:
            random_float = np.random.uniform(lower_bound, upper_bound)
            if random_float not in arr_set:
                yield random_float
                generated_count += 1
    ...
    new_scores = list(generate_missing_floats(scores, lower_bound=0.0, upper_bound=100.0, num_to_generate=100))
    labels  = [args.class_index] * len(new_scores)
    ...
    if args.on == 'real':
        ...
        if args.regression_models == 'STGCN':
                    ...
                    generated_samples = generator.generate_by_class(device = args.device,gif_directory=stgcn_run_dir_class,scores=new_scores,class_index=args.class_index)
                    gen_set = Kimore(generated_samples,labels,np.array(new_scores))
                    ...
                    print(len(new_scores),len(y_pred))
                    for i in range(len(y_pred)): 
                        print(f'true score {new_scores[i]:.2f}, predicted score {y_pred[i]:.2f}')
                    print(rmse(y_pred,new_scores))
                    print('MAE',mae(y_pred,new_scores))
