...
class CVAE(nn.Module):
    def __init__(self,output_directory,epochs,device,latent_dimension=16,num_classes=5,hid_dim=16,mlp_dim=16,filters=128,lr=1e-4,w_kl=1e-3,w_rec=0.999):
        ...
        self.fc = nn.Linear(self.latent_dimension+self.mlp_dim, self.filters*606)
        self.deconv6 = nn.ConvTranspose1d(self.filters,self.filters, kernel_size=3)
        self.deconv5 = nn.ConvTranspose1d(self.filters,self.filters, kernel_size=5)
        self.deconv4 = nn.ConvTranspose1d(self.filters,self.filters, kernel_size=10)   
        self.deconv3 = nn.ConvTranspose1d(self.filters,self.filters, kernel_size=20)
        self.deconv2 = nn.ConvTranspose1d(self.filters,self.filters,kernel_size=40)
        self.deconv1 = nn.ConvTranspose1d(self.filters,self.filters, kernel_size=70)
    ...
    def train_step(self, x, label, optimizer):
        ...
        reconstructed_samples = self.decode(latent_space, label)
...
#ADD MEAN AND VARIANCE 2D VISUALIZATION.
    ...
    def generate_skeleton(self, device, label):
        ...
        with torch.no_grad():
                generated_sample = model.decode(sample, c).cpu().double().numpy()
