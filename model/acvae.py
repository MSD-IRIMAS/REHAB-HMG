...
class ACVAE(nn.Module):
    """The following model is Action conditioned VAE only (without the score) trained on the whole data"""
    def __init__(self, output_directory, epochs, device, latent_dimension=256, num_classes=5, hid_dim=16, mlp_dim=16, filters=128, lr=1e-4, w_kl=1e-3, w_rec=0.999):
...
#ADD MEAN AND VARIANCE 2D VISUALIZATION.
    ...
    def generate_samples_from_prior(self, device, class_index, gif_directory,num_samples):
        self.device = device
        self.to(device)
