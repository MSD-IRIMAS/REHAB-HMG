...
class ASCVAE(nn.Module):
    """This model is conditioned with the exercise category and the score and it is trained on the whole dataset"""
...
#ADD MEAN AND VARIANCE 2D VISUALIZATION.
    ...
    def generate_by_class(self,device,scores,class_index,gif_directory):
    ...
    def evaluate_function(self, dataloader, device):
        ...
        with torch.no_grad():
            for batch_idx, (data, label, score) in enumerate(dataloader):
                ...
                reconstructed_samples, mu, log_var = self.forward(data, label, score)
