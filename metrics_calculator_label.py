        ...
        for class_index in range(args.class_index):
                ...
                def get_generator():
                        if args.generative_model == 'CVAEL':
                                ...
                                generator= VAE(output_directory=output_directory_skeletons_class, device=args.device, epochs=2000)
                ...
                generated_samples, test_generated_samples = get_generator()
                        
                    
                gen_set = Kimore(generated_samples, ytrain, strain)
                gen_loader = DataLoader(gen_set, batch_size=10, shuffle=True)
                print(len(xtrain))

                gen_set = Kimore(test_generated_samples, ytest, stest)
                test_gen_loader = DataLoader(gen_set, batch_size=10, shuffle=True)
